...
public:
  explicit Impl(Mouse* owner) noexcept(false)
    : mState{}, mOwner(owner), mIs4k(false), mConnected(0), mDeviceToken(0),
    mMode(MODE_ABSOLUTE), mScrollWheelCurrent(0), mRelativeX(INT64_MAX),
    mRelativeY(INT64_MAX), mRelativeWheelY(INT64_MAX) {
    if (s_mouse)
    {
    ...
    ThrowIfFailed(mGameInput->RegisterDeviceCallback(
      nullptr, GameInputKindMouse, GameInputDeviceConnected,
  ...
  void GetState(State& state) const
  {
    ...
    if (state.positionMode == MODE_RELATIVE)
    {
      ...
      if (SUCCEEDED(mGameInput->GetCurrentReading(GameInputKindMouse, nullptr,
...
private:
  ...
  static void CALLBACK OnGameInputDevice(
    _In_ GameInputCallbackToken, _In_ void* context, _In_ IGameInputDevice*,
    _In_ uint64_t, _In_ GameInputDeviceStatus currentStatus,
...
void Mouse::ProcessMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  DWORD result =
    WaitForSingleObjectEx(pImpl->mScrollWheelValue.get(), 0, FALSE);
  if (result == WAIT_FAILED)
    throw std::exception("WaitForSingleObjectEx");
...
public:
  explicit Impl(Mouse* owner) noexcept(false)
    : mState{}, mOwner(owner), mWindow(nullptr), mMode(MODE_ABSOLUTE),
    mLastX(0), mLastY(0), mRelativeX(INT32_MAX), mRelativeY(INT32_MAX),
    mInFocus(true) {
    if (s_mouse)
    {
      throw std::exception("Mouse is a singleton");
...
void Mouse::ProcessMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  HANDLE events[3] = { pImpl->mScrollWheelValue.get(),
                      pImpl->mAbsoluteMode.get(), pImpl->mRelativeMode.get() };
...
class Mouse::Impl {
public:
  explicit Impl(Mouse* owner) noexcept(false) : mOwner(owner)
  {
    if (s_mouse)
    {
      throw std::exception("Mouse is a singleton");
...
//
// For a Windows Store app or Universal Windows Platform (UWP) app, add the
// following to your existing application methods:
//
...
public:
  explicit Impl(Mouse* owner) noexcept(false)
    : mState{}, mOwner(owner), mDPI(96.f),
    mMode(MODE_ABSOLUTE), mPointerPressedToken{}, mPointerReleasedToken{},
    mPointerMovedToken{}, mPointerWheelToken{}, mPointerMouseMovedToken{} {
    if (s_mouse)
    {
      throw std::exception("Mouse is a singleton");
    }
  ...
  void SetMode(Mode mode)
  {
    ...
    else
    {
      if (!mCursor)
      {
        ...
        hr = factory->CreateCursor(CoreCursorType_Arrow, 0,
                                   mCursor.GetAddressOf());
        ThrowIfFailed(hr);
      }
  ...
  bool IsConnected() const
  {
    ...
    HRESULT hr = RoActivateInstance(
      HStringReference(RuntimeClass_Windows_Devices_Input_MouseCapabilities)
      .Get(),
      &caps);
    ThrowIfFailed(hr);
  ...
  void SetWindow(ABI::Windows::UI::Core::ICoreWindow* window)
  {
    ...
    hr = window->add_PointerWheelChanged(
      Callback<PointerHandler>(PointerWheel).Get(), &mPointerWheelToken);
    ThrowIfFailed(hr);
  }
...
private:
  ...
  static HRESULT PointerEvent(IInspectable*,
  ...
  static HRESULT PointerWheel(IInspectable*,
                              ABI::Windows::UI::Core::IPointerEventArgs* args)
  {
    using namespace ABI::Windows::Foundation;
    using namespace ABI::Windows::UI::Input;
